<aside class="main-sidebar">
        <!-- sidebar: style can be found in sidebar.less -->
        <section class="sidebar">
          <!-- Sidebar user panel -->
          
          <!-- search form -->
        
          <!-- /.search form -->
          <!-- sidebar menu: : style can be found in sidebar.less -->
          <ul class="sidebar-menu">
            
            <li class="active treeview">
              <a href="dashboard.php">
                <i class="fa fa-dashboard"></i> <span>Dashboard</span> <i class="fa fa-angle-left pull-right"></i>
              </a>
              
            </li>
            
             <li class="treeview">
              <a href="">
                <i class="fa fa-angle-left pull-right"></i>
                <span>Maverick Rewards</span>                
              </a>     
                <ul class="treeview-menu">
                <li><a href="maverick-rewards.php"><i class="fa fa-circle-o"></i> Maverick Rewards</a></li>  
                 
                
                
              </ul>
                 </li>   
            
            
                  <li class="treeview">
              <a href="">
                <i class="fa fa-angle-left pull-right"></i>
                <span>Maverick Games</span>                
              </a>     
                <ul class="treeview-menu">
                <li><a href="maverick-games.php"><i class="fa fa-circle-o"></i> Maverick Games</a></li> 
              </ul>
                 </li>   
            
                      
                 
             
           
            
	  	
           
             
              	
	 
	 
           <li class="treeview">
              <a href="">
                <i class="fa fa-angle-left pull-right"></i>
                <span>Users</span>                
              </a>     
                <ul class="treeview-menu">
                <li><a href="register-users.php"><i class="fa fa-circle-o"></i> Register Users</a></li>
                   
                
              </ul>
                 </li>   
             <!--
            <li class="treeview">
              <a href="">
                <i class="fa fa-angle-left pull-right"></i>
                <span>Stories</span>                
              </a>     
                <ul class="treeview-menu">
                    <li><a href="stories.php"><i class="fa fa-circle-o"></i>Stories</a></li>
                <li><a href="story_comments.php"><i class="fa fa-circle-o"></i> Story Comments</a></li>                
              </ul>
                 </li>
              -->
             

          </ul>
        </section>
        <!-- /.sidebar -->
      </aside>